...
library(knitr)
opts_chunk$set(fig.path='figure/pos-', fig.align='center', fig.show='hold', size="footnotesize",            fig.width=8, fig.height=5, out.width='.95\\textwidth', warning=FALSE)
@
...
<<>>=
plot(GEN.G(), range=c(250,450))
@

<<>>=
plot(GEN.T(), range=c(250,450))
@

\newpage

<<>>=
plot(GEN.M(), range=c(250,450))
@
...
<<>>=
plot(DNA.N(), range=c(250,450))
@

<<>>=
plot(DNA.P(), range=c(250,450))
@
\newpage

<<>>=
plot(DNA.GM(), range=c(250,450))
@
...
my.plotter <- function(bwfs.fun, w.low=250, w.high=450, ylab="Action"){
  spectrum.data <- data.frame(w.length=seq(w.low, w.high, length.out=200))
...
<<plot-GEN-M>>=
my.plotter(GEN.M.q.fun, ylab="GPAS 'Micheletti' action (quantum)")
@
