...
\section{Waveband constructors for ranges}

Functions for several colour bands, in some cases according to different
optional definitions, are listed in Tables \ref{tab:uv:unweighted},
\ref{tab:vis:unweighted}, \ref{tab:ir:unweighted}, and \ref{tab:landsat:unweighted}.
...
\begin{table*}
    \caption[Unweighted UV wavebands]{Functions in R package \PBWB used for constructing
    descriptors of ultraviolet wavebands used for calculation of irradiances or exposures. The boundaries of
    the band given as wavelengths in nm ($\lambda$). Definition according to ISO-21348 "ISO" is
    the default for all functions except \texttt{UVA1()} and \texttt{UVA2()} for which there is only one definition in common use "CIE" which is the default.}\label{tab:uv:unweighted}\vspace{2ex}
    \centering
\textsf{%
       \begin{small}
       \begin{tabular}{@{}lllc@{}}
      ...
      \UVB & ISO-21348 & \texttt{UVB("ISO")} & $280\leq\lambda<315$ \\
       ...
       \UVA & CIE & \texttt{UVA("CIE")} & $315\leq\lambda<400$ \\
       \UVA & n.a. & \texttt{UVA("none")} & $320\leq\lambda<400$ \\
       \midrule
       \UVA{}1 & CIE & \texttt{UVA1("CIE")} & $315\leq\lambda<340$ \\
       \midrule
       \UVA{}2 & CIE & \texttt{UVA2("CIE")} & $340\leq\lambda<400$ \\
       \midrule
...
\begin{table*}
    \caption[Unweighted VIS wavebands]{Functions in R package \PBWB used for constructing
    descriptors of visible wavebands used for calculation of irradiances or exposures. The boundaries of
    the band given as wavelengths in nm ($\lambda$). Definition according to ISO-21348 "ISO" is
    the default for all functions except \texttt{PAR()} for which there is only one definition
    in common use "Plant" which is the default.}\label{tab:vis:unweighted}\vspace{2ex}
    \centering
\textsf{%
       \begin{small}
       \begin{tabular}{@{}lllc@{}}
...
\begin{table*}
    \caption[Unweighted IR wavebands]{Functions in R package \PBWB used for constructing
    descriptors of infrared wavebands used for calculation of irradiances or exposures. The boundaries of
    the band are given as wavelengths in nm ($\lambda$). Definition according to ISO-20473, "ISO" is
    the default for all functions except \texttt{IRA()}, \texttt{IRB()}, \texttt{IRC()} are defined
    based on CIE recommendations.}\label{tab:ir:unweighted}\vspace{2ex}
    \centering
\textsf{%
       \begin{small}
       \begin{tabular}{@{}lllc@{}}
       ...
       Infrared & & & \\
       \midrule
       IR & ISO-20473 & \texttt{IR("ISO")} & $780\leq\lambda<1 \times 10^6$ \\
       NIR & ISO-20473 & \texttt{NIR("ISO")} & $780\leq\lambda<3000$ \\
       MIR & ISO-20473 & \texttt{MIR("ISO")} & $3000\leq\lambda<50000$ \\
       FIR & ISO-20473 & \texttt{FIR("ISO")} & $5000\leq\lambda<1 \times 10^6$ \\
       IR & CIE & \texttt{IR("CIE")} & $700\leq\lambda<1 \times 10^6$ \\
       IR-A & CIE & \texttt{IRA("CIE")} & $780\leq\lambda<1400$ \\
       IR-B & CIE & \texttt{IRB("CIE")} & $1400\leq\lambda<3000$ \\
       IR-C & CIE & \texttt{IRC("CIE")} & $3000\leq\lambda<1 \times 10^6$ \\
       SWIR &  n.a. & \texttt{SWIR()} & $1400\leq\lambda<3000$ \\
       \midrule
      \textsl{Arbitrary} & n.a. & \texttt{new\_waveband(lo,hi)} & $lo\leq\lambda<hi$ \\
       \bottomrule
     \end{tabular}
...
\begin{table*}
    \caption[Landsat wavebands]{Functions in R package \PBWB used for constructing
    descriptors of Landsat-imager wavebands. The boundaries of
    the band are given as wavelengths in nm ($\lambda$). Definitions follow NASA and USGS documentation for boundaries at half maximum (HM).}\label{tab:landsat:unweighted}\vspace{2ex}
    \centering
\textsf{%
       \begin{small}
       \begin{tabular}{@{}lllc@{}}
       \toprule
       Waveband & imager & constructor & Wavelengths (nm) \\
       \midrule
       Landsat & & & \\
       \midrule
       Purple/Coastal & OLI & \texttt{Purple("LandsatOLI")} & $433\leq\lambda<453$ \\
       \midrule
      Blue & TM & \texttt{Blue("LandsatTM")} & $450\leq\lambda<520$ \\
      Blue & ETM & \texttt{Blue("LandsatETM")} & $450\leq\lambda<520$ \\
      Blue & OLI & \texttt{Blue("LandsatOLI")} & $450\leq\lambda<515$ \\
       \midrule
      Green & RBV & \texttt{Green("LandsatRBV")} & $480\leq\lambda<580$ \\
      Green & MSS & \texttt{Green("LandsatMSS")} & $500\leq\lambda<600$ \\
      Green & TM & \texttt{Green("LandsatTM")} & $520\leq\lambda<600$ \\
      Green & ETM & \texttt{Green("LandsatETM")} & $520\leq\lambda<600$ \\
      Green & OLI & \texttt{Green("LandsatOLI")} & $525\leq\lambda<600$ \\
       \midrule
      Red & RBV & \texttt{Red("LandsatRBV")} & $580\leq\lambda<680$ \\
      Red & MSS & \texttt{Red("LandsatMSS")} & $600\leq\lambda<700$ \\
      Red & TM & \texttt{Red("LandsatTM")} & $630\leq\lambda<690$ \\
      Red & ETM & \texttt{Red("LandsatETM")} & $630\leq\lambda<690$ \\
      Red & OLI & \texttt{Red("LandsatOLI")} & $630\leq\lambda<680$ \\
       \midrule
      NIR & RBV & \texttt{NIR("LandsatRBV")} & $700\leq\lambda<830$ \\
      NIR & MSS & \texttt{NIR("LandsatMSS")} & $800\leq\lambda<1100$ \\
      NIR & TM & \texttt{NIR("LandsatTM")} & $760\leq\lambda<900$ \\
      NIR & ETM & \texttt{NIR("LandsatETM")} & $760\leq\lambda<900$ \\
      NIR & OLI & \texttt{NIR("LandsatOLI")} & $845\leq\lambda<885$ \\
       \midrule
      SWIR1 & TM & \texttt{SWIR("LandsatTM")} & $1550\leq\lambda<1750$ \\
      SWIR1 & ETM & \texttt{SWIR("LandsatETM")} & $1550\leq\lambda<1750$ \\
      SWIR1 & OLI & \texttt{SWIR("LandsatOLI")} & $1560\leq\lambda<1660$ \\
       \midrule
      SWIR2 & TM & \texttt{SWIR("LandsatTM")} & $2080\leq\lambda<2350$ \\
      SWIR2 & ETM & \texttt{SWIR("LandsatETM")} & $2080\leq\lambda<2350$ \\
      SWIR2 & OLI & \texttt{SWIR("LandsatOLI")} & $2100\leq\lambda<2300$ \\
       \midrule
      TIR1 & TIRS & \texttt{TIR1("LandsatTIRS")} & $10600\leq\lambda<11200$ \\
       \midrule
      TIR2 & TM & \texttt{TIR2("LandsatTM")} & $10400\leq\lambda<12500$ \\
      TIR2 & ETM & \texttt{TIR2("LandsatETM")} & $10400\leq\lambda<12500$ \\
      TIR2 & TIRS & \texttt{TIR2("LandsatTIRS")} & $11500\leq\lambda<12500$ \\
       \bottomrule
     \end{tabular}
     \end{small}
}\end{table*}

Constructors of lists of waveband definitions frequently used together are also defined in the package (Table \ref{tab:uw:lists}).
...
\textsf{%
       ...
       UV defs. & CIE & \texttt{UV\_bands("CIE")} & "CIE" \\
       UV defs. & n.a. & \texttt{UV\_bands("medical")} & "medical" \\
       UV defs. & n.a. & \texttt{UV\_bands("none")} & "none" \\
       VIS defs. & ISO-21348 & \texttt{VIS\_bands("ISO")} & "ISO" \\
       IR defs. & ISO-20473 & \texttt{IR\_bands("ISO")} & "ISO" \\
       IR defs. & CIE & \texttt{IR\_bands("CIE")} & "CIE" \\
       ...
       Plant sens. & n.a. & \texttt{Plant\_bands("")} & "ISO" and "McCree" \\
       Plant sens. & n.a. & \texttt{Plant\_bands("energy")} & "ISO" and "McCree" \\
       Plant sens. & n.a. & \texttt{Plant\_bands("photon")} & "ISO" and "McCree" \\
       Plant sens. & n.a. & \texttt{Plant\_bands("ISO")} & "ISO" and "McCree" \\
       Plant sens. & n.a. & \texttt{Plant\_bands("CIE")} & "CIE" and "McCree" \\
       Landsat & NASA/USGS & \texttt{Landsat\_bands("L8")} & Landsat 8 mission \\
       Landsat & NASA/USGS & \texttt{Landsat\_bands("L$x$")} & Landsat $ x = 1\ldots 8 $ missions \\
       \bottomrule
     \end{tabular}
     \end{footnotesize}
}\end{table*}
...
The waveband definitions and SWFs are stored in \texttt{waveband} objects, that can be created
...
The luminous flux per unit are in lux can be calculated as follows using the original luminous efficiency function for the human eye using for defining the lumen. As we start with spectral irradiance we obtain luminous flux per unit area expressed in lux.
